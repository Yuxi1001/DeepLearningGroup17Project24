...
class MambaEmbeddingsForCEHR(nn.Module):
    ...
    def __init__(
        ...
        self.linear_projection = nn.Linear(input_dim, self.hidden_size)
    ...
    # ) -> Any:
        ...
        inputs_embeds = input_ids.permute(0, 2, 1)
        # print('input shape:', inputs_embeds.shape)

        ## time embeddings #############################################
        # Process time_stamps with a linear layer
        time_stamps_embeds = self.time_embeddings(time_stamps)
        # print('time shape:', time_stamps_embeds.shape)
        # Flatten the last two dimensions of time_stamps_embeds
        # batch_size, seq_length, _, _ = time_stamps_embeds.shape
        # time_stamps_embeds = time_stamps_embeds.view(batch_size, seq_length, -1)

        ## age embeddings #############################################
        # ages = ages.unsqueeze(-1).unsqueeze(-1)
        # ages = ages.expand(-1, input_ids.size(1), -1)  
        ...
        # print("word_embeddings.shape:", inputs_embeds.shape)
        # print("position_embeddings.shape:", position_embeddings.shape)
        # print("time_stamps_embeds.shape after reshaping:", time_stamps_embeds.shape)
        # print("age_embeddings.shape:", ages_embeds.shape)
