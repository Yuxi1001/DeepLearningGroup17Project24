from EHRMamba2 import Mamba2Pretrain
...
@click.option(
    "--model_type", default="transformer", help="Specify model type (e.g., transformer, EHRMamba2)"
)
...
# Model Initialization based on model_type argument
if model_type == "EHRMamba2":
    # Instantiate Mamba2Pretrain with CLI-provided parameters
    model = Mamba2Pretrain(
        vocab_size=30522,  # Assuming default vocab size; customize as needed
        ...
        padding_idx=0,  # Customize if necessary
        cls_idx=1,
        eos_idx=2,
        n_groups=1,
        chunk_size=256  # Customize based on EHRMamba2 requirements
    )
else:
    # Retain existing model initialization for other types
    model = <EXISTING_MODEL_INITIALIZATION_CODE>  # Placeholder for existing code
